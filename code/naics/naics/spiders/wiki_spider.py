...
class WikiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        i = 0
        rows = response.xpath(
            '//*[@id="mw-content-text"]/div/table[@class="infobox vcard" ]/tbody/tr')
        for row in rows:
            ...
            if row.css('.image'):
                if row.css('img'):
                    ...
                    item = 'Logo_{}'.format(i)
                    try:
                        value['logo_thumb_url'] = row.css('img').xpath(
                            '@src').extract_first().replace('//', '')
                        try:
                            value['logo_url'] = domain + \
                                row.css('a::attr(href)').extract_first()[1:]
                            try:
            ...
            # Scraping info box values
                ...
                item = [_.strip()
                        for _ in item if _.strip() and _.replace('\n', '')]
                item = ' '.join(item)
                item = item.replace('\n', '')
